          ...
          //  for (let i = 0; i < articles.length; i++) {
            ...
            for (let i = 0; i < 15; i++) {
                ...
                if (messageText) {
                    ...
                    const unwantedKeywords = ['Sent', 'You sent', 'Enter', 'Facebook', 'end-to-end encrypted', 'Messages and calls are secured with end-to-end encryption. Learn more'];

                    // Retrieve the last two meaningful messages
                    for (let j = messageLines.length - 1; j >= 0; j--) {
                        const line = messageLines[j];
                        if (!unwantedKeywords.some(keyword => line.includes(keyword))) {
                            if (lastMessage === '') {
